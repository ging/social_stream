...
-export([start/2, stop/1, on_presence/4, on_remove_connection/3, on_unset_presence/4, isConnected/1, on_packet_send/3]).
...
on_presence(User, _Server, _Resource, _Packet) ->
    ?INFO_MSG("mod_sspresence: on_presence", []),
    %?INFO_MSG("User vale: ~p", [User]),
    Login_path = string:concat(getOptionValue("scripts_path="), "/presence_script "),
    os:cmd(string:join([Login_path, User ], " ")),
    ok.

on_remove_connection(_SID, _JID, _SessionInfo) ->
    %?INFO_MSG("on_remove_connection!!", []), 
    ok.

on_unset_presence(User, _Server, _Resource, _Status) ->
    ?INFO_MSG("mod_sspresence: on_unset_presence", []),
    %?INFO_MSG("User vale: ~p", [User]),
    %?INFO_MSG("Server vale: ~p", [Server]),

    Connected = isConnected(User),

    case Connected of
	true -> ok;
	_ -> Logout_path = string:concat(getOptionValue("scripts_path="), "/unset_presence_script "),
...
on_packet_send(From, _To, {xmlelement, Type, _Attr, Subel} = _Packet) ->
	...
	"message" -> 
		{_SenderJID,Sender,_SenderDomain,_A,_B,_C,_D} = From,
    		%{_ReceiverJID,Receiver,ReceiverDomain,_E,_F,_J,_H} = To,
		%?INFO_MSG("Sender vale: ~p", [Sender]),
...
getMessageFromSubel(Subel) ->
    ...
    MessageString = binary_to_list(list_to_binary(io_lib:format("~p", [MessageData]))),
